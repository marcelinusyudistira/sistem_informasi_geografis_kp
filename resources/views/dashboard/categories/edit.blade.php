    ...
    <div class="col-lg-8">
        ...
        <form action="/dashboard/categories/{{ $category->id }}" method="POST" class="mb-5">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('cat_name') is-invalid @enderror" id="cat_name" name="cat_name" autofocus value="{{ old('cat_name', $category->cat_name) }}">
                {{-- Pesan Error --}}
                @error('cat_name')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="icon_parent">
                    @foreach ($links as $link)    
                        @if(old('icon_parent', $category->icon_parent) == $link->code )
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="icon_child">
                    @foreach ($links as $link)    
                        @if(old('icon_child', $category->icon_child) == $link->code )
                            <option selected value="{{ $link->code }}">{{ $link->nama }}</option>
                        @else
                            <<option value="{{ $link->code }}">{{ $link->nama }}</option>
                        @endif
